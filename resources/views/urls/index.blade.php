    ...
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('urls.store') }}">
            ...
            <input type="number"
                ...
                min="2"
                max="10"
                placeholder="{{ __('Desired Length') }}"
                class="block w-full border-gray-300 mt-5 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm"
                value="{{ old('desired_length') }}"
            />
            <x-input-error :messages="$errors->store->get('desired_length')" class="mt-2" />
            ...
            @foreach ($urls as $item)
                <div class="p-6 flex space-x-2">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="mt-4 text-lg text-gray-900">{{ __('shortened Url: ') }}
                            <a href="{{ route('shortened-url', $item->shortened_url) }}" target="_blank">
                                {{ route('shortened-url', $item->shortened_url) }}
                            </a>
                        </p>
                        @php
                            $character_count=strlen($item->original_url)
                        @endphp
                        <p class="mt-4 text-lg text-gray-900">{{ __('character count: ') }}{{ $character_count}}</p>
                    </div>
                </div>
            @endforeach
        </div>
